<template>
  <div
    class="flex items-center justify-center gap-2 px-4 py-3 bg-gray-200 border-b border-gray-300 text-gray-800 text-lg font-semibold my-2">
    <FileText class="w-6 h-6 text-gray-700" />
    <span>Amaliyot: <span class="text-gray-900 font-bold">{{ id }}</span></span>
  </div>
    ...
    methods: {
      async loadPdf() {
        try {
          ...
          const pdfPath = new URL(`../assets/amaliy/${this.id}-amaliy.pdf`,
            import.meta.url);
